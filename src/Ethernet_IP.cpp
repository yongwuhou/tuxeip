...
Eip_CPF *_GetEipCPF(Encap_Header *header)
{
	...
	switch (header->Command) {
		...
		case EIP_LISTINTERFACES:
			return ((Eip_CPF*)((char*)header+sizeof(Encap_Header)));
		case EIP_SENDRRDATA:
		case EIP_SENDUNITDATA:
			return((Eip_CPF*)((char*)header+sizeof(SendRRData_Request)));
		default:
			return(NULL);
	}
}
...
Eip_Session *_OpenSession(char *serveur,int port,int timeout)
{
	...
	CIPERROR(0,0,0);
	session=(Eip_Session *)malloc(sizeof(Eip_Session));
	if (session==NULL) {
...
int _RegisterSession(Eip_Session *session)
{
	...
	if ((res=CipSendData(session->sock,(Encap_Header*)&request))<0)
		return(res);
	...
	if (reply->Header.Status==EIP_SUCCESS)
...
int _GetItemSize(Eip_Item *item)
{
	switch (item->Type_Id) {
		case ItemId_Null:
			return(sizeof(Eip_NAI));
		case ItemId_ConnectionBased:
			return(sizeof(Eip_CAI));
		case ItemId_ConnectedTP:
			return(sizeof(Eip_CDI));
		case ItemId_UCM:
			return(sizeof(Eip_UDI));
		case ItemId_OTSocketInfo:
		case ItemId_TOSocketInfo:
			return(sizeof(Eip_SII));
		case ItemId_Sequenced:
			return(sizeof(Eip_SAI));
		default:
			return(0);
	}
}
...
int _AddItem(Encap_Header *request,Eip_Item *item,void *data)
{
	...
	int datasize=0;
	if (taille==0) {
		CIPERROR(Internal_Error,E_ItemUnknown,0);
		return(Error);
	}
	...
	if (data!=NULL) {
		...
		if (item->Length>0)
			_AddHeader(request,data,datasize);
	} else {
		_AddHeader(request,item,taille+datasize);
	}
	return(item->Length);
}
	...
	void *request,int size)
{
	...
	if (reply!=NULL) {
		...
		/* Result is OK */
      CIPERROR(EIP_Error,reply->Status,0);
		result=(Eip_CDI *)malloc(sizeof(Eip_CDI)+respdataitem->Length);
...
{
	char *pos;
	if (size>(MAX_MSG_LEN-header->Length-sizeof(*header)))
		return(0);
