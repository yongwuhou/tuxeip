...
{ char *adr=NULL;
	...
	PCCC_SLCRW_Request *request=(PCCC_SLCRW_Request *)malloc(rsize);
	if (request==NULL)
	{
...
{ int rsize=0;
	...
	request=(PCCC_SLCRW_Request *)malloc(rsize);
	if (request==NULL)
	{
		CIPERROR(Sys_Error,errno,0);
		if (requestsize!=NULL) *requestsize=0;
...
{ int dhpsize=0;
  ...
  LogCip(LogTrace,"->Entering SendUnConnectedPCCCRequest \n");
	...
	reply=(PCCC_Header *)malloc(Datasize);
	if (reply==NULL)
	{
		CIPERROR(Sys_Error,errno,0);
		free(mrreply);
...
{ int datasize=0;
	...
	*type=(PLC_Data_Type)0;*tsize=0;*esize=0;
	...
	if ((flag>>4)<8)
	{
		*type=(PLC_Data_Type)(flag>>4);
	}	else
	{
		datasize =((flag>>4)&0x07);
		switch (datasize)
		{
			case 1:	*type=(PLC_Data_Type)*((BYTE*)data);
							data+=1;
							break;
			case 2:	*type=(PLC_Data_Type)*((WORD*)data);
							data+=2;
							break;
			case 4:	*type=(PLC_Data_Type)*((LONGWORD*)data);
							data+=4;
							break;
			default:*type=(PLC_Data_Type)0;*tsize=0;*esize=0;return(NULL);
		}
	}
	if ((flag&0x0f)<8)
	{
...
{ PLC_Read *result=NULL;
	...
	result=(PLC_Read *)malloc(sizeof(PLC_Read));
	if (result==NULL)
	{
		CIPERROR(Sys_Error,errno,0);
		return(NULL);
	...
	if ((flag>>4)<8)
	{
		result->type=(PLC_Data_Type)(flag>>4);
	}	else
	{
		datasize =((flag>>4)&0x07);
		switch (datasize)
		{
			case 1:	result->type=(PLC_Data_Type)*((BYTE*)data);
							data+=1;
							break;
			case 2:	result->type=(PLC_Data_Type)*((WORD*)data);
							data+=2;
							break;
			case 4:	result->type=(PLC_Data_Type)*((LONGWORD*)data);
							data+=4;
							break;
			default:result->type=(PLC_Data_Type)0;result->totalsize=0;result->elementsize=0;return(0);
		}
	}
	if ((flag&0x0f)<8)
	{
	...
	result=(PLC_Read *)realloc(result,sizeof(PLC_Read)+result->totalsize);
	if (result==NULL)
	{
		CIPERROR(Sys_Error,errno,0);
		return(NULL);
...
{ float value=0;
 ...
 void _FreePLCRead(PLC_Read *Data)
{
		free(Data);
}
