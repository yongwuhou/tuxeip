...
{ char *Path=NULL; //was void*
	...
	ReadDataService_Request *request=(ReadDataService_Request *)malloc(rsize);
	if (request==NULL)
	{
...
{ char *Path=NULL; // was Void
	...
	WriteDataService_Request *request=(WriteDataService_Request *)malloc(rsize);
	if (request==NULL)
	{
		CIPERROR(Sys_Error,errno,0);
		if (requestsize!=NULL) *requestsize=0;
...
{ CommonDataService_Reply *rep=NULL;
    ...
    switch (dataitem->Type_Id)
    {
			...
			case (ItemId_ConnectedTP):rep=(CommonDataService_Reply *)((char*)dataitem+sizeof(Eip_CDI));
												break;
			default:CIPERROR(Internal_Error,E_ItemUnknown,0);return(0);
    }
	switch (rep->Service)
		{
...
{ CommonDataService_Reply *rep=NULL;
	...
	switch (dataitem->Type_Id)
	{
		...
		case (ItemId_ConnectedTP):rep=(CommonDataService_Reply *)((char*)dataitem+sizeof(Eip_CDI));
											break;
		default:CIPERROR(Internal_Error,E_ItemUnknown,0);return(NULL);
	}
	switch (rep->Service)
	{
		case (CIP_MULTI_REQUEST+0x80) :
...
{	if (reply==NULL) {CIPERROR(Internal_Error,E_InvalidReply,0);return((LGX_Data_Type)0);}
	if (reply->Status!=0) {CIPERROR(AB_Error,reply->Status,_GetExtendedStatus(reply));return((LGX_Data_Type)0);}
	switch (reply->Service)
	{
		case (CIP_DATA_READ+0x80):
		{	CIP_INT *type;
			type=(CIP_INT*)(((char*)reply)+sizeof(CommonDataService_Reply));
			return((LGX_Data_Type)*type);
		}
		default:CIPERROR(Internal_Error,E_UnsupportedDataType,0);return((LGX_Data_Type)0);
	}
}
