	...
	MR_Request *MRrequest,int MRrequestsize,BYTE *routepath,CIP_USINT routepathsize)
{
	...
	Unconnected_Send_Request *request=(Unconnected_Send_Request *)malloc(taille);
	if (request==NULL) {
...
Eip_Connection *_Forward_Open(Eip_Session *session,BYTE Priority,
			...
			BYTE *path,CIP_USINT requestpathsize)
{
	...
	Forward_Open_Request *request=(Forward_Open_Request *)malloc(sizeof(Forward_Open_Request)+2*taille);
	if (request==NULL) {
		CIPERROR(Sys_Error,errno,0);
		return(NULL);
	}
	...
	request->Priority=Priority; /* used to calculate request timeout information */
	request->TimeOut_Ticks=TimeOut_Ticks; /* used to calculate request timeout information */
	request->OT_ConnID=0; /* originator's CIP Produced session ID */
	...
	free(request);
	if (reply!=NULL) {
		Success_Forward_Open_Reply *sfor=(Success_Forward_Open_Reply *)_GetMRData(reply);
		if (sfor!=NULL) {
			Eip_Connection *connection=(Eip_Connection *)malloc(sizeof(Eip_Connection)+2*requestpathsize);
			if (connection==NULL) {
				CIPERROR(Sys_Error,errno,0);
				free(reply);
				return(NULL);
	...
	CIP_UDINT OriginatorSerialNumber,BYTE *path,CIP_USINT requestpathsize)
{
	...
	int replysize=0;
	Forward_Close_Request *request=(Forward_Close_Request *)malloc(requestsize);
	if (request==NULL) {
		CIPERROR(Sys_Error,errno,0);
		return(Error);
	}
...
int _Forward_Close(Eip_Connection *connection)
{
	...
	BYTE *routepath,CIP_USINT routepathsize,int *replysize)
{
	...
	if ((routepath!=NULL) && (routepathsize>0)) {
		int UCRequestsize=sizeof(Unconnected_Send_Request)
			...
			+(MRrequestsize%2) // pad
			+routepathsize+routepathsize%2;
		UCRequest=(Unconnected_Send_Request *)malloc(UCRequestsize);
		if (UCRequest==NULL) {
			CIPERROR(Sys_Error,errno,0);
			return(NULL);
		};
